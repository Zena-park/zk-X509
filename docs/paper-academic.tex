...
hashes} (country, organization, organizational unit, common name). These
...
into the OS keychain, where the private key is managed by the secure
hardware (macOS Secure Enclave or software keystore).
...
hierarchy: KISA Root CA → Authorized CA (e.g., \begin{CJK}{UTF8}{mj}금융결제원\end{CJK}) → User
Certificate.
...
disclosure hash fields enable selective attribute disclosure (Section
3.11) --- each field is either the SHA-256 hash of the corresponding
certificate attribute salted with a private-key-derived salt (when
...
The prover server scans the OS keychain for identities (certificate +
...
--- the prover server receives only the resulting signature bytes and
...
even the prover server's process memory never contains the key.
...
Secure Enclave, Windows TPM, or software keystore). This architectural
...
\tightlist
\item
  \textbf{Signing ($\sim$1 second).} The prover server
  generates \texttt{ownership\_sig} and \texttt{nullifier\_sig} using
  the OS keychain. The private key is erased from memory immediately
  after signing.
\item
  \textbf{Proving ($\sim$10 minutes CPU).} The SP1 zkVM
...
If proof generation were offloaded to a third party, these secrets would
be exposed. In zk-X509, the private key never leaves the user's device
at any stage --- the zkVM witness contains only signatures and publicly
derivable certificate data. This represents a strictly stronger security
model than existing ZK identity systems.
...
bridges \\
Selective disclosure & \texttt{countryHash} directly reveals
jurisdiction, reducing anonymity set to single-country users &
User-controlled --- do not disclose unless required \\
...
\tightlist
\item
  \textbf{Smart contract unit tests} (Foundry): 82 test cases across two
  test suites --- IdentityRegistry (70 tests) covering registration,
  re-registration, double-registration prevention, registrant mismatch,
  CA Merkle root validation with grace period, timestamp validation
  ...
  prevention), CRL Sorted Merkle root validation, pause/unpause, max
  proof age adjustment, minimum disclosure mask enforcement, and
  two-step ownership management; and RegistryFactory (12 tests) covering
  registry creation, metadata tracking, beacon proxy deployment, and
  access control.
\item
...
\textbf{Private Key Isolation} & Yes (key never in circuit) & No (DKIM
key in circuit) & No & No & No & N/A \\
...
unique architectural advantage --- no other system keeps the private key
entirely outside the ZK circuit - \textbf{SP1 cycle count (11.8M)} is
