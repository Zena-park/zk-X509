...
\item \textit{Selective disclosure} (salted with private-key-derived salt):\\
  $\text{salt} \leftarrow \mathcal{H}(\text{``zk-X509-Disclosure-Salt-v1''} \| \text{nullifier\_sig})$;\\
  for each masked bit: $\text{hash} \leftarrow \mathcal{H}(\text{field} \| \text{salt})$ or $\texttt{0x0}$
...
\texttt{countryHash} & \texttt{bytes32} & $\mathcal{H}(\text{country} \| \text{salt})$ or \texttt{0x0} \\
\texttt{orgHash} & \texttt{bytes32} & $\mathcal{H}(\text{organization} \| \text{salt})$ or \texttt{0x0} \\
\texttt{orgUnitHash} & \texttt{bytes32} & $\mathcal{H}(\text{org unit} \| \text{salt})$ or \texttt{0x0} \\
\texttt{commonNameHash} & \texttt{bytes32} & $\mathcal{H}(\text{common name} \| \text{salt})$ or \texttt{0x0} \\
\bottomrule
\end{tabular}
\end{table}
...
\midrule
\texttt{sp1Verifier} & \texttt{ISP1Verifier} (immut.) & On-chain proof verifier \\
\texttt{programVKey} & \texttt{bytes32} (immut.) & ZK program verification key \\
\texttt{maxWalletsPerCert} & \texttt{uint32} (immut.) & Max wallets per certificate \\
\texttt{caMerkleRoot} & \texttt{bytes32} & Merkle root of allowed CA set \\
...
\texttt{minDisclosureMask} & \texttt{uint8} (immut.) & Minimum required disclosure fields \\
\bottomrule
\end{tabular}
\end{table}
...
\item $\mathcal{A}$ outputs: two registration transactions $\text{tx}_1 = (\pi_1, \text{pubvals}_1)$ from $\text{addr}_1$ and $\text{tx}_2 = (\pi_2, \text{pubvals}_2)$ from $\text{addr}_2$
