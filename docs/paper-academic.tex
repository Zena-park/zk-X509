...
\fi
\usepackage{lmodern}
\usepackage{CJKutf8}  % Korean character support
\ifPDFTeX\else
...
\author{Yeong Ju Bak\\
Tokamak Network\\
\texttt{yj@tokamak.network}}

\date{March 2026\\{\small arXiv preprint --- v1}}
...
Meanwhile, over four billion active X.509 digital certificates
...
delegated to the OS keychain (macOS Secure Enclave, Windows TPM). The
circuit verifies six properties: (1) full certificate chain validity to
...
timestamp, and four optional selective disclosure hashes. On-chain
...
establish six properties via game-based definitions: unforgeability,
unlinkability, double-registration resistance, front-running immunity,
CA anonymity, and non-transferability, with explicit reductions to
...
\noindent\textbf{Keywords:} Zero-Knowledge Proofs, X.509, Digital Identity, zkVM,
Ethereum, Korean NPKI, Privacy-Preserving Authentication, Proof of
Personhood
...
Existing approaches to on-chain identity fall into three categories,
...
\item
  ...
  \cite{ref3} depends on proprietary biometric scanners (the Orb). These
...
Signatures Act \cite{ref27}. These certificates embed RSA or ECDSA signatures from
trusted CAs, providing a cryptographic chain of trust that can be
verified computationally---and therefore inside a zero-knowledge
circuit. The core insight of zk-X509 is that these existing credentials,
already trusted by governments and institutions, can serve as blockchain
identity anchors \emph{today}, without waiting years for new DID
...
\textbf{expiry time} (\texttt{notAfter}), a \textbf{chain ID} (EIP-155 \cite{ref28}),
a \textbf{registry address} (cross-DApp unlinkability), a \textbf{CRL
Merkle root}, and four optional \textbf{selective disclosure hashes}
(country, organization, organizational unit, common name). These are
committed as public outputs and verified on-chain by a Solidity smart
contract.
...
as a concrete case study. Korean digital certificates (\begin{CJK}{UTF8}{mj}공인인증서\end{CJK}) are
issued by authorized CAs such as the Korea Financial Telecommunications
and Clearings Institute (\begin{CJK}{UTF8}{mj}금융결제원\end{CJK}), employing a 3-level certificate
chain (Root CA $\rightarrow$ Intermediate CA $\rightarrow$ User Certificate) with RSA-2048 and
SHA-256 or SHA-1 signatures. Private keys use PBES2 encryption with
PBKDF2-HMAC-SHA1 and SEED-CBC or AES-256-CBC ciphers.
...
($\sim$1.3M e-residents), German eID---all operating under the eIDAS regulation \cite{ref26}---and corporate PKI
systems---each user proving identity under their national CA without any
cross-border credential issuance. The \texttt{caMerkleRoot} in the
public values attests that the certificate was issued by one of the
whitelisted CAs without revealing which one, preserving jurisdictional
privacy. Applications requiring jurisdiction-specific logic can request
the user to additionally disclose \texttt{countryHash} via selective
disclosure (Section 3.10).
...
\item
  ...
  the Dolev-Yao adversary model, establishing six properties ---
  unforgeability, unlinkability, double-registration resistance,
  front-running immunity, CA anonymity, and non-transferability --- with
...
hierarchy: KISA Root CA $\rightarrow$ Authorized CA (e.g., \begin{CJK}{UTF8}{mj}금융결제원\end{CJK}) $\rightarrow$ User
Certificate.
...
the statement \cite{ref5}. The random oracle model \cite{ref15} provides a standard framework for analyzing hash-based constructions in such proofs. Formally, a ZK proof system \((P, V)\) for a
language \(L\) satisfies three properties: \textbf{completeness} (honest
provers convince honest verifiers), \textbf{soundness} (no cheating
prover can convince on false statements), and \textbf{zero-knowledge}
...
computation was performed correctly. The development of succinct non-interactive arguments of knowledge (SNARKs) \cite{ref14} established the foundation for this approach, with subsequent systems such as Groth16 \cite{ref7}, PLONK \cite{ref16}, Bulletproofs \cite{ref31}, and STARKs \cite{ref17} offering different trade-offs between proof size, verification time, and trusted setup requirements. Zerocash \cite{ref32} demonstrated the practical application of SNARKs to privacy-preserving blockchain transactions.

\textbf{SP1} (Succinct Processor 1) is a RISC-V-based zkVM developed by Succinct \cite{ref6}, alongside alternatives such as RISC Zero \cite{ref29}. It compiles standard Rust to RISC-V instructions executed inside the zkVM, enabling complex operations such as ASN.1 parsing and RSA verification. SP1 provides precompiled accelerators for SHA-256 and RSA modular exponentiation, and supports on-chain verification via Groth16 \cite{ref7} or PLONK \cite{ref16} proof systems. Circuit-level DSLs such as Circom \cite{ref30} offer an alternative approach but lack the general-purpose programmability of zkVMs.
...
\textbf{zkPassport} \cite{ref2} generates ZK proofs of passport data read
via NFC. While similar in spirit to zk-X509, it requires NFC hardware
and is limited to passports. zk-X509 is purely software-based and works
with any X.509 certificate.

\textbf{Worldcoin} \cite{ref3} uses iris biometric scanning with a
proprietary device (the Orb) to generate unique identity proofs. The
hardware dependency and biometric data collection raise both
accessibility and privacy concerns that zk-X509 avoids entirely.

\textbf{Polygon ID and DID-based systems} \cite{ref13} use W3C Verifiable
...
\textbf{Anonymous credential systems.} The foundational work of Chaum \cite{ref19} introduced the concept of privacy-preserving transactions, and Camenisch and Lysyanskaya \cite{ref18} formalized anonymous credential schemes based on bilinear maps. These systems enable selective attribute disclosure but require purpose-built credential issuance---they cannot leverage existing X.509 certificates. zk-X509 achieves comparable privacy properties by applying ZK proofs to credentials that already exist at scale.

\textbf{Proof of Personhood.} Ford \cite{ref20} and Borge et al.\ \cite{ref21} formalized the concept of proof of personhood as a prerequisite for digital democracy. zk-X509 instantiates this concept using existing government-issued certificates rather than novel pseudonym party protocols or biometric approaches.
...
\bottomrule\noalign{}
\endlastfoot
zkPassport \cite{ref2} & Passport & NFC required & Government CA & Yes
(passports) & N/A & Full ZK \\
Worldcoin \cite{ref3} & Biometric & Orb required & Worldcoin Foundation & No
& N/A & Partial \\
...
Decentralized Identifier (DID) frameworks \cite{ref13} represent the
...
(e.g., Korea's Electronic Signatures Act \cite{ref27}). This eliminates the ``who
trusts whom?'' bootstrapping problem.
...
are legally binding under the Electronic Signatures Act \cite{ref27}. This makes
zk-X509 immediately applicable to compliance-sensitive domains (banking,
...
the prover (user), \(\mathcal{S}\) the prover server (localhost daemon
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{struct PublicValuesStruct \{}
\NormalTok{    bytes32 nullifier;       // H(Sign(sk, H("zk{-}X509{-}Nullifier{-}v2" ‖ registryAddress ‖ chainId)) ‖ walletIndex)}
\NormalTok{    bytes32 caMerkleRoot;    // Merkle root of allowed CA set (hides which CA issued the cert)}
\NormalTok{    uint64  timestamp;       // Unix timestamp at proof generation}
\NormalTok{    address registrant;      // Wallet address bound to this proof}
\NormalTok{    uint32  walletIndex;     // Wallet slot index (0..maxWalletsPerCert{-}1)}
\NormalTok{    uint64  notAfter;        // Certificate expiry (unix timestamp)}
\NormalTok{    uint64  chainId;         // EIP{-}155 chain ID}
\NormalTok{    address registryAddress; // Target registry contract address}
\NormalTok{    bytes32 crlMerkleRoot;   // CRL Sorted Merkle Tree root (bytes32(0) = disabled)}
\NormalTok{    bytes32 countryHash;     // H(country ‖ salt) or 0x0 if not disclosed}
\NormalTok{    bytes32 orgHash;         // H(organization ‖ salt) or 0x0 if not disclosed}
\NormalTok{    bytes32 orgUnitHash;     // H(organizational unit ‖ salt) or 0x0 if not disclosed}
\NormalTok{    bytes32 commonNameHash;  // H(common name ‖ salt) or 0x0 if not disclosed}
\NormalTok{\}}
\end{Highlighting}
\end{Shaded}
...
issued the certificate (Section 3.11). The \texttt{walletIndex} field
...
The \texttt{chainId} and \texttt{registryAddress} fields enable
cross-chain replay prevention and cross-DApp unlinkability --- each
deployment produces different nullifiers. The \texttt{crlMerkleRoot}
field commits the CRL Sorted Merkle Tree root, enabling on-chain
validation of revocation checking (\texttt{bytes32(0)} disables CRL
enforcement). The four disclosure hash fields enable selective attribute
disclosure (Section 3.10) --- each field is either the SHA-256 hash of
the corresponding certificate attribute salted with a
private-key-derived salt (when disclosed) or zero (when hidden),
controlled by the user's \texttt{disclosure\_mask}.
...
user's private key never enters the zkVM}. Instead, the prover server
...
proof for CA membership (Section 3.11) \\
...
crlMerkleRoot, and four selective disclosure hashes (countryHash,
...
chain: user cert $\rightarrow$ intermediate CAs $\rightarrow$ root CA. For single-level PKI (no
...
importing the private key into the zkVM, the prover server signs a
...
ECDSA (P-256, P-384 with RFC 6979 \cite{ref22} deterministic nonces). The key type is
auto-detected from the certificate's SPKI algorithm OID: RSA keys use
direct \texttt{rsa} crate verification, while ECDSA keys use the
\texttt{p256}/\texttt{p384} crates with the curve determined from the
SPKI \texttt{namedCurve} parameter.

This approach has three advantages: (1) the private key never exists in
the prover server's process memory---only the OS keychain handles it at
...
and (3) the timestamp binding prevents a compromised prover server from
...
key. RSA PKCS\#1 v1.5 and ECDSA with RFC 6979 \cite{ref22} deterministic nonces are
both inherently deterministic --- the same key always produces the same
signature, ensuring nullifier consistency. The ZK circuit verifies the
\texttt{nullifier\_sig} against the certificate's public key before
computing the nullifier.
...
The \texttt{maxWalletsPerCert} parameter is set at deployment, enabling
...
\item
  ...
  \texttt{caMerkleRoot\ ==\ contract.caMerkleRoot}
\item
...
immutably at contract deployment:
...
\subsection{NPKI Integration}\label{npki-integration}

\textbf{Certificate Discovery.} The prover server includes an NPKI
filesystem scanner that automatically discovers certificate/key pairs in
platform-specific directories:
\texttt{$\sim${}/Library/Preferences/NPKI} (macOS),
\texttt{$\sim${}/.pki/NPKI} (Linux), and
\texttt{\%LOCALAPPDATA\%\textbackslash{}NPKI} (Windows). For each
discovered pair (\texttt{signCert.der} + \texttt{signPri.key}), the
scanner extracts metadata (subject, issuer, serial number, expiry) for
display in the frontend's certificate selection UI. This eliminates the
need for manual file upload.

\textbf{Private Key Decryption.} Korean NPKI private keys are stored in
PKCS\#8 EncryptedPrivateKeyInfo format using PBES2 with PBKDF2-HMAC-SHA1
key derivation. Two encryption ciphers are supported:

\begin{itemize}
\tightlist
\item
  \textbf{SEED-CBC} (OID 1.2.410.200004.1.4): The Korean national block
  cipher, widely used in legacy NPKI certificates. Supported via the
  \texttt{kisaseed} crate.
\item
  \textbf{AES-256-CBC} (OID 2.16.840.1.101.3.4.1.42): Used in newer NPKI
  certificates.
\end{itemize}

The decryption module: (1) parses the ASN.1 encryption parameters, (2)
derives the key via PBKDF2, (3) decrypts using the appropriate cipher,
and (4) strips PKCS\#7 padding to yield the raw PKCS\#1 DER private key.
A generic \texttt{decrypt\_cbc\textless{}C\textgreater{}()} function
handles both ciphers uniformly.
...
\subsection{Delegated Proving
Architecture}\label{delegated-proving-architecture}
...
both generated on the user's local device. This architectural separation
of signing and proving enables \textbf{delegated proving} as the
recommended deployment model:

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\item
  \textbf{User (phone/browser):} Generates \texttt{ownership\_sig} and
  \texttt{nullifier\_sig} using the OS keychain ($\sim$1
  second). The private key remains in the Secure Enclave / TPM.
\item
  \textbf{Cloud prover (untrusted):} Receives the signatures +
  certificate (public data) and generates the SP1 proof using GPU
  acceleration ($\sim$1--2 minutes). The cloud never sees the
  private key.
\item
  \textbf{User:} Submits the proof to the blockchain via
  \texttt{register()}.
\end{enumerate}
...
\begin{itemize}
\item
  \textbf{Performance.} Local CPU proving takes $\sim$10
  minutes. Cloud GPU proving reduces this to $\sim$1--2
  minutes, making the system practical for interactive workflows.
\item
  \textbf{CRL enforcement.} The cloud prover downloads the latest CRL
  directly from the CA and includes it in the proof. Unlike local
  proving where the user can omit the CRL, the cloud prover enforces CRL
  checking --- a user with a revoked certificate cannot bypass
  revocation by simply not providing the CRL.
\item
  \textbf{Security.} Even if the cloud prover is malicious, it cannot
  forge signatures (no private key), register to a different wallet
  (address-bound \texttt{ownership\_sig}), or reuse the proof later
  (timestamp-bound) or on another chain (chain\_id-bound).
\end{itemize}

\textbf{Comparison with other systems.} In zk-email and Semaphore,
private inputs (DKIM key, secret identity) enter the circuit directly,
making delegation impossible without exposing sensitive data. zk-X509's
signature-based architecture is unique in enabling privacy-preserving
delegation.
...
1 & Organization (O) & 2.5.4.10 & ``\begin{CJK}{UTF8}{mj}금융결제원\end{CJK}'', ``Samsung'' \\
2 & Organizational Unit (OU) & 2.5.4.11 & ``\begin{CJK}{UTF8}{mj}개인\end{CJK}'', ``Engineering'' \\
3 & Common Name (CN) & 2.5.4.3 & (user's name --- typically hidden) \\
\end{longtable}

For each bit set in the mask, the circuit extracts the corresponding
field from the certificate's subject DN, hashes it with a
\textbf{private-key-derived salt}, and commits the hash as a public
value. For unset bits, zero is committed. The salt is computed as
\(\mathcal{H}(\text{"zk-X509-Disclosure-Salt-v1"} \| \text{nullifier\_sig})\),
where \texttt{nullifier\_sig} is the deterministic signature used for
nullifier generation. This salt is deterministic (same certificate
always produces the same salt) yet private (only the private key holder
can compute \texttt{nullifier\_sig}), preventing brute-force attacks on
small input spaces such as country codes ($\sim$200 values).
Without this salt, an attacker could precompute
\(\mathcal{H}(\text{"KR"})\), \(\mathcal{H}(\text{"US"})\), etc., and
match against on-chain \texttt{countryHash} values. With the salt, each
user's hashes are unique and unpredictable.
...
DAO voting contract may require only \texttt{countryHash}, while a
corporate DeFi protocol may additionally require \texttt{orgHash}. The
user decides what to reveal on a per-proof basis.

\textbf{Privacy guarantee.} Fields with mask bit = 0 produce a zero hash
...
\item
  \textbf{Proof generation.} The prover server computes
...
\textbf{Merkle tree construction.} A standard binary SHA-256 Merkle tree \cite{ref25}
with sorted-pair hashing is used: \(H(\min(a,b) \| \max(a,b))\).
Sorted-pair hashing prevents second preimage attacks and eliminates the
need for direction bits in the proof path. For \(n\) whitelisted CAs,
the proof consists of \(\lceil \log_2 n \rceil\) hashes (e.g., 4 hashes
for 16 CAs), adding negligible overhead to the ZK circuit
($\sim${}\(\log_2 n\) additional SHA-256 computations).
...
\paragraph{4.3.1 Off-Chain Cost: ZK
...
\paragraph{4.3.3 End-to-End Latency}\label{end-to-end-latency}
...
\bottomrule\noalign{}
\endlastfoot
NPKI key decryption (PBES2) & \textless{} 1 second \\
SP1 execute (no proof, CPU) & $\sim$15 seconds \\
SP1 prove (CPU, single-level) & $\sim$10 minutes \\
SP1 prove (GPU, estimated) & $\sim$1--2 minutes \\
...
\tightlist
\item
  \textbf{Smart contract unit tests} (Foundry): 17 test cases covering
  registration, re-registration, double-registration prevention,
  registrant mismatch, CA Merkle root validation, timestamp validation
  (future and stale proofs), chain ID mismatch, registry address
  mismatch, wallet index out-of-range, certificate expiry, nullifier
  revocation, CA management (add, batch add, remove, duplicate
  prevention), CRL Merkle root validation, pause/unpause, max proof age
  adjustment, and two-step ownership management.
\item
...
\item
  ...
  deployment + prover server + frontend registration, verified with
  \texttt{cast} commands.
\end{enumerate}
...
independently detected from the signer's SPKI \texttt{namedCurve} OID \cite{ref23},
correctly handling the RFC 5758 separation of concerns.
...
\item
  ...
  prover server as a localhost daemon on their own machine.
\item
...
\item
  ...
  (e.g., \begin{CJK}{UTF8}{mj}금융결제원\end{CJK}) whose root public key hash is whitelisted in
  \(\mathcal{V}\).
\end{itemize}

\textbf{Localhost assumption.} The prover server operates as a local
daemon on the user's physical machine. Private keys transit only within
the local loopback interface (\texttt{127.0.0.1}), never over the public
network. This ensures that the private key exposure surface is limited
...
\item
  ...
  factoring assumption \cite{ref12}; ECDSA is secure under the elliptic curve
  discrete logarithm assumption; SHA-256 is collision-resistant and
  preimage-resistant. Signature schemes satisfy EUF-CMA security \cite{ref24}.
\item
...
We formalize six security properties using game-based definitions. In
...
fundamental limitation shared by all credential systems (Section 7.5).
...
(Section 3.11). This significantly enlarges the anonymity set in
...
\textbf{Theorem 7 (Cross-Service Unlinkability).} For any two
...
unforgeability (sEUF-CMA) and key privacy --- i.e., signatures on
distinct messages under different keys are computationally
indistinguishable from random, which holds for RSA PKCS\#1 v1.5 and
ECDSA with RFC 6979 under standard hardness assumptions.
...
\item
  ...
  both nullifiers.
\item
  \textbf{Linking \(\sigma_1\) to \(\sigma_2\) without knowing \(sk\).}
  Suppose \(\mathcal{A}\) can determine whether two signatures on
  distinct messages were produced by the same key. We construct an
  adversary \(\mathcal{B}\) against EUF-CMA security: \(\mathcal{B}\)
  queries the signing oracle on \(d_1\) and \(d_2\), obtains
  \(\sigma_1^*\) and \(\sigma_2^*\), and uses \(\mathcal{A}\)'s linking
  strategy. If \(\mathcal{A}\) can correlate signatures without the key,
  \(\mathcal{B}\) gains information about the signing key beyond what is
  revealed by EUF-CMA-secure signatures on chosen messages. Under the
  assumption that the signature scheme provides key privacy (implicit in
  EUF-CMA for deterministic schemes like RSA PKCS\#1 v1.5 and ECDSA with
  RFC 6979, where signature distributions are key-dependent but
  computationally indistinguishable across keys on distinct messages),
...
\textbf{Theorem 8 (Cross-Chain Replay Resistance).} A valid proof for
...
\paragraph{Theorem 3 (Double-Registration
Resistance)}\label{theorem-3-double-registration-resistance}

\emph{Under assumption A4 (ZK soundness) and the determinism of SHA-256,
...
\paragraph{Theorem 4 (Front-Running
Immunity)}\label{theorem-4-front-running-immunity}

\emph{Under assumption A4 (ZK soundness) and A1 (local security),
zk-X509 satisfies front-running immunity (Definition 4).}
...
\item
  \textbf{Generate a new proof.} \(\mathcal{A}\) would need to execute
  the ZK circuit with \(sk\) as a witness to produce a valid proof
  binding to \(\text{addr}_A\). By A1, \(\mathcal{A}\) does not have
  \(sk\).
\end{enumerate}

All strategies fail. \(\square\)

\paragraph{Theorem 5 (CA-Membership
...
only Korean NPKI), CA anonymity is trivially satisfied (anonymity set =
...
\paragraph{Theorem 6
(Non-Transferability)}\label{theorem-6-non-transferability}
...
\item
  ...
  \(\text{Sig.Verify}(\text{cert.pk}, \mathcal{H}(\text{"zk-X509-Nullifier-v2"} \| \text{contract\_address} \| \text{chain\_id}), \text{nullifier\_sig})\).
...
\paragraph{5.5.1 Timestamp Manipulation}\label{timestamp-manipulation}
...
\paragraph{5.5.2 CRL Integrity and
...
\paragraph{5.5.3 Private Key Isolation}\label{private-key-isolation}
...
\tightlist
\item
  The prover server decrypts the NPKI private key file using the user's
  password.
\item
  The decrypted key is passed to the OS keychain signing API (macOS
  Security.framework, Windows CNG).
\item
  The OS keychain signs the ownership challenge:
  \(\mathcal{H}(\text{serial} \| \text{registrant} \| \text{wallet\_index} \| \text{timestamp} \| \text{chain\_id})\).
\item
  Only the resulting \textbf{signature bytes} are passed to the SP1 zkVM
  as input.
\item
  The private key is immediately erased from memory after signing.
\end{enumerate}
...
hardware. - CORS restricted to \texttt{localhost:3000}; no
\texttt{Debug} derive on key-holding structs.

This represents a strictly stronger security model than the typical
approach of importing the private key directly into the ZK circuit, and
exceeds the trust model of standard certificate-using software (e.g.,
web browsers performing TLS client authentication).

\paragraph{5.5.4 Smart Contract Security}\label{smart-contract-security}
...
\item
  ...
  a two-step pattern (\texttt{transferOwnership} $\rightarrow$
...
Non-transferability & Enforced (without cooperation) & Theorem 6;
...
All measurements were taken on the same machine (macOS, Apple Silicon)
for fair comparison.
...
Groth16 & Circom + Groth16 & Noir/Circom & Custom \\
\textbf{Constraints / Cycles} & 11.8M cycles (P-256) & 1.26M constraints
(measured) & $\sim$1M constraints (docs) & $\sim$150K
constraints (docs) & N/A & N/A \\
\textbf{Proof Generation} & 102s (CPU, multi-core) & TBD & TBD & TBD &
TBD & TBD \\
\textbf{Trusted Setup} & Not required (STARK) & Required (Groth16) &
Required & Required & TBD & N/A \\
\textbf{On-Chain Gas} & $\sim$300K (est. Groth16) &
$\sim$300K (est. Groth16) & $\sim$350K (docs) &
$\sim$150K (docs) & $\sim$250K (est.) &
$\sim$200K (est.) \\
\textbf{Hardware Required} & None & None & None & None & NFC reader &
Orb biometric \\
\textbf{PKI Compatibility} & Any X.509 CA & DKIM (email only) & DID only
& None (custom) & Passport chip & None \\
\textbf{Credential Source} & Government PKI & Email providers & New DID
issuers & None & Passport & Biometric \\
\textbf{Privacy Level} & Full (Merkle CA) & Partial (reveals domain) &
Selective disclosure & Group membership & Partial & Iris hash \\
\textbf{Delegated Proving} & Yes (key never in circuit) & No (DKIM key
in circuit) & No & No & No & N/A \\
\textbf{Cross-DApp Unlinkability} & Yes (contract-bound nullifier) & No
& Yes & Yes (group-scoped) & No & No \\
...
any X.509 CA worldwide with full CA anonymity - \textbf{zk-email} has
...
issuance infrastructure - \textbf{zk-X509's delegated proving} is a
unique architectural advantage --- no other system allows untrusted
cloud proving without privacy loss - \textbf{SP1 cycle count (11.8M)} is
...
The current architecture requires a localhost prover server. While the
...
\subsection{Prover Marketplace
Integration}\label{prover-marketplace-integration}

The delegated proving architecture described in Section 3.10 can be
extended to decentralized prover marketplaces (e.g., Succinct Network,
Gevulot) where multiple competing provers bid on proof generation. This
would further reduce costs through market competition and eliminate
...
strictly stronger security model than existing ZK identity systems. The
...
The security analysis under the Dolev-Yao model establishes six
...
to EUF-CMA security and ZK zero-knowledge), double-registration
resistance (via deterministic nullifiers and ZK soundness),
front-running immunity (via registrant binding), CA anonymity (via
