...
such as Korean NPKI, separate CAs issue certificates for different
entity types (e.g., personal vs.\ business). Since each registry
maintains a CA whitelist via \texttt{addCA()}, entity-level filtering
is achieved by registering only the appropriate CA hashes---for example,
adding only business-class CA hashes to enforce ``only business entities
may register.'' For finer-grained control, the \texttt{requiredOrg}
field constraint can restrict registration to a specific organization,
verified inside the ZK proof without requiring public disclosure.
