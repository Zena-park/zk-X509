...
into the OS keychain, where the private key is managed by the software
keystore (login keychain). Hardware-backed storage via the Secure Enclave
is available only for keys generated on the device, not for imported keys.
...
\textbf{Trusted Setup} & Groth16: circuit-specific ceremony (Aztec Ignition); PLONK: universal & Required (per-circuit) &
Required & Required & Not required (Ultra Honk) & N/A \\
